...
gcloud run deploy air-api `
  ...
  --region $REGION `
  --project $PROJECT_ID `
  --set-env-vars "R_RUNTIME_URL=$R_URL,SESSION_BUCKET=$BUCKET,DATA_STORE_ID=r-docs-curated,SEARCH_LOCATION=global,GOOGLE_CLOUD_PROJECT=$PROJECT_ID,GOOGLE_CLOUD_LOCATION=us-central1" `
  --quiet
